 ...
 * rank, the next distinct score jumps by the size of the tie group. Pulse is
 * minutes studied (focus mode + timer/study-room sessions; manual logs are
 * excluded upstream) plus 30 per topic moved forward. `adjustments` (userId
 * -> pulse delta, e.g. admin pulse adjustments) are applied before ranking;
 * the stored pulse includes them.
 */
