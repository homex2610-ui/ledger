...
import {
  getGetActiveAnnouncementQueryKey,
  useCreateReferralAttribution,
  useDismissAnnouncement,
  useGetActiveAnnouncement,
  useGetMe,
  useLogOut,
    ...
    <div className="min-h-[100dvh] bg-background md:h-[100dvh] md:overflow-hidden">
      <div className="main-grid mx-auto grid min-h-[100dvh] max-w-[1540px] md:h-full md:overflow-hidden">
        <aside className="hidden border-r border-sidebar/30 bg-sidebar px-4 py-6 text-sidebar-foreground md:flex md:flex-col md:overflow-y-auto">
          <Link href="/" className="mb-6 flex items-center gap-3" data-testid="link-brand">
            <BrandMark size={36} />
            <span>
              <span className="block font-display text-[1.15rem] font-bold tracking-tight">PrepPulse</span>
              <span className="font-mono-custom text-[9px] uppercase tracking-[.2em] text-sidebar-foreground/55">
                smart prep companion
              </span>
            </span>
          </Link>
            ...
            {navigation.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link
                  ...
                  className={`nav-link relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold hover:translate-x-0.5 ${
                    active
                      ? 'bg-sidebar-foreground/10 text-sidebar-foreground shadow-sm'
                      : 'text-sidebar-foreground/65 hover:bg-sidebar-foreground/7 hover:text-sidebar-foreground'
                  }`}
                >
                  {active && (
                    <span className="absolute left-0 top-1/2 h-5 w-[3px] -translate-y-1/2 rounded-full bg-accent glow-accent" />
                  )}
          ...
          {me?.profile.isAdmin && (
            <Link
              ...
              className={`nav-link relative mt-1 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold hover:translate-x-0.5 ${
            ...
            >
              ...
              <ShieldCheck
                size={17}
                strokeWidth={location.startsWith('/admin') ? 2.5 : 1.8}
                className={location.startsWith('/admin') ? 'text-accent' : ''}
              />
              Admin
            </Link>
          )}
          ...
          <Link
            ...
            className="nav-link mt-4 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-sidebar-foreground/65 hover:bg-sidebar-foreground/7 hover:text-sidebar-foreground"
          >
          ...
          <button
            ...
            className="nav-link mt-1 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-sidebar-foreground/65 hover:bg-sidebar-foreground/7 hover:text-sidebar-foreground"
        ...
        <div className="min-w-0 md:flex md:h-full md:min-h-0 md:flex-col md:overflow-hidden">
          <header className="sticky top-0 z-10 flex h-14 items-center justify-between border-b border-border/70 bg-background/90 px-5 backdrop-blur-md md:px-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-mono-custom text-[10px] uppercase tracking-[.19em] text-muted-foreground">
                  {dateLabel}
                </p>
            ...
            <div className="flex items-center gap-3">
              {activeAnnouncement && (
                ...
                >
                  ...
                  <span className="max-w-[240px] truncate" title={activeAnnouncement.body}>
                  ...
                  <button
                    ...
                    className="shrink-0 rounded-full p-0.5 text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground"
                  >
                    <X size={13} />
                  </button>
                </div>
              ...
              <Link
                ...
                className="flex items-center gap-3 rounded-full pl-2 transition-transform hover:scale-[1.02]"
              >
                <span className="hidden text-right sm:block">
                  <span className="block text-[11px] font-bold leading-tight">{handle}</span>
                  <span className="font-mono-custom text-[9px] text-muted-foreground">
                    {examLabel} · {me?.profile.targetYear ?? ''}
                  </span>
                </span>
                <Avatar
                  src={me?.profile.avatarUrl}
                  initials={initials}
                  className="h-9 w-9 bg-primary text-xs text-white"
                  title={handle}
                />
              </Link>
            </div>
          ...
          {reminder.show && location !== '/study' && (
            <div
              className="mx-5 mt-4 flex items-center gap-3 rounded-2xl border border-primary/25 bg-primary/5 px-4 py-3 md:mx-10"
            ...
            >
              ...
              <Link
                ...
                data-testid="link-reminder-study"
              >
                Start a block
              </Link>
              <button
          ...
          <main className="page-enter px-5 pb-28 pt-6 md:min-h-0 md:flex-1 md:overflow-y-auto md:px-10 md:pb-12 md:pt-8">
            {children}
          </main>
        </div>
      </div>
      ...
      <nav
        className="fixed inset-x-4 bottom-4 z-30 flex items-center justify-around rounded-2xl border border-border/80 bg-card/95 p-2 shadow-[0_14px_40px_hsl(186_32%_16%/.12)] backdrop-blur-md md:hidden"
        ...
        {navigation.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              className={`flex min-w-[40px] flex-col items-center gap-0.5 rounded-xl px-1 py-1.5 text-[9px] font-semibold ${
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-sidebar/40 md:hidden" onClick={() => setMobileMenuOpen(false)}>
          <div
            className="h-full w-[82%] max-w-[300px] bg-sidebar p-6 text-sidebar-foreground shadow-2xl"
          ...
          >
            ...
            <nav className="space-y-2">
              {navigation.map(({ href, label, icon: Icon }) => (
                <Link
                  ...
                  href={href}
                  onClick={() => setMobileMenuOpen(false)}
                  data-testid={`link-drawer-${label.toLowerCase()}`}
                  className={`flex items-center gap-3 rounded-xl px-3 py-3 font-semibold ${
                    location === href ? 'bg-accent/20 text-accent' : 'text-sidebar-foreground/70'
                  }`}
                >
                  <Icon size={18} />
                  {label}
              ...
              {me?.profile.isAdmin && (
                <Link
                  href="/admin"
                  onClick={() => setMobileMenuOpen(false)}
                  data-testid="link-drawer-admin"
                  className={`flex items-center gap-3 rounded-xl px-3 py-3 font-semibold ${
                    location.startsWith('/admin') ? 'bg-accent/20 text-accent' : 'text-sidebar-foreground/70'
                  }`}
                >
                  <ShieldCheck size={18} />
                  Admin
            ...
            <Link
              href="/settings"
              onClick={() => setMobileMenuOpen(false)}
              data-testid="link-drawer-settings"
              className="mt-3 flex items-center gap-3 rounded-xl px-3 py-3 font-semibold text-sidebar-foreground/70"
            >
              <Settings size={18} />
              Settings
            </Link>
            <button
              ...
              data-testid="button-drawer-sign-out"
              className="mt-1 flex w-full items-center gap-3 rounded-xl px-3 py-3 font-semibold text-sidebar-foreground/70"
            >
              <LogOut size={18} />
              Sign out
            </button>
