...
function TopicRow({ topic, pending, onUpdate }: { topic: Topic; pending: boolean; onUpdate: (id: string, status: string) => void }) {
  ...
  return <div className="group flex flex-col gap-2.5 border-b border-border/60 p-4 transition-colors duration-200 last:border-b-0 hover:bg-secondary/25 focus-within:bg-secondary/25 md:px-5"><div className="flex items-center justify-between gap-4"><div className="flex min-w-0 flex-wrap items-center gap-2"><p className="truncate text-sm font-bold text-foreground md:text-[15px]" title={topic.name}>{topic.name}</p><span className={`shrink-0 rounded-full px-2 py-0.5 font-mono-custom text-[9px] font-medium uppercase tracking-wide ${topic.weightage === 'high' ? 'bg-pop/15 text-pop' : 'bg-secondary text-muted-foreground'}`}>{topic.weightage}</span></div><div className="relative shrink-0"><Select disabled={pending} value={topic.status} onChange={(event) => onUpdate(topic.id, event.target.value)} className={`h-9 rounded-lg border-0 px-3 pr-7 text-xs font-bold shadow-sm ${statusTone[topic.status]}`} data-testid={`select-topic-status-${topic.id}`} icon={<ChevronDown size={13} className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2" />}>{options.map((value) => <option key={value} value={value}>{statusLabel[value]}</option>)}</Select></div></div><ProgressBar value={statusWeight[topic.status] ?? 0} className="h-1.5 w-full" color={topic.status === 'mastered' ? 'success' : 'warm'} /><div className="flex items-center gap-2"><span className="font-mono-custom text-[10px] text-muted-foreground">{topic.accuracy}% accuracy</span><span className="h-0.5 w-0.5 rounded-full bg-muted-foreground/40" /><span className="font-mono-custom text-[10px] text-muted-foreground">{topic.questionCount} Qs</span></div></div>;
}